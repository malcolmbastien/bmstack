$(document).ready(function() {
    ...
    var print_field_list = function(data, block) {
    	...
    	//	}
    		...
    		$(list_item + ' > .field-text > .note-option').click(function () {
    				...
    				$(list_item + ' > input.field-text').keypress(function(e) {
            ...
            // Set color Blue
    		$(list_item + ' > div > div > span.note-blue').click(function () {
    		    $(list_item).css('background-color','#48A8C0');
    ...
    var submit_canvas = function(action) {
        var project_id = $('#project_id').text();
		var account = $('#account').text();
		var canvas_set_id = $('#canvas_set_id').text();
		var notes = $('#id_canvas-notes').val();
		var canvas_id = $('#canvas_id').text();
		var tags = $('#id_canvas-tags').val();
		$.post('/save_add_canvas/', { canvas_data:canvas_data, canvas_id:canvas_id, action:action, account:account, notes:notes, tags:tags, project_id:project_id, canvas_set_id:canvas_set_id}, function(data) {
    ...
    if (data) { 
        for (i in data) {
    //        alert(data[i]['pk']);
            var block = data[i]['fields']['block'];
            var position = canvas_data[block].length;
            canvas_data[block].push({});
            canvas_data[block][position]['id'] = temp_id;
            canvas_data[block][position]['text'] = data[i]['fields']['text'];
            canvas_data[block][position]['colour'] = data[i]['fields']['colour'];
            canvas_data[block][position]['note'] = data[i]['fields']['note'];
            temp_id++;
        }
    }
    canvas_blocks.forEach(function(item) {
